...
#
# Hinweis zur ZIP-Auswahl:
#   Eingabe ja   → jede Kategorie wird als ZIP-Datei gespeichert
#   Eingabe nein → kein ZIP wird erstellt
...
while True:
    zip_input = input("Sollen die Kategorien als ZIP-Dateien gespeichert werden? (ja/nein): ").strip().lower()
    if zip_input in ["ja", "nein"]:
        ZIP_KATEGORIEN = zip_input == "ja"
        break
    else:
        print("Bitte gib 'ja' oder 'nein' ein.")

while True:
    dup_input = input("Wie sollen Duplikate behandelt werden? (immer/nie/fragen): ").strip().lower()
    if dup_input in ["immer", "nie", "fragen"]:
        DUPLICATE_BEHAVIOR = dup_input
        break
    else:
        print("Bitte gib 'immer', 'nie' oder 'fragen' ein.")
...
with open(FINISH_LOG, "w", encoding="utf-8") as flog:
    flog.write(f"Erkannte Verknüpfungen: {len(links)}\n")
...
<body>
    ...
    <p><strong>Erstellt von:</strong> JeromeX</p>
    <p><strong>Datum:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
    <p><strong>Gesamt-Zuordnungen:</strong> {len(links)}</p>
    <p class='success'><strong>Kopiert:</strong> {copied_count}</p>
    <p class='skipped'><strong>Übersprungene Duplikate:</strong> {skipped_duplicates}</p>
    <h2>ZIP-Downloads pro Kategorie</h2>
    <ul>
""")
    for cat_id, folder in path_map.items():
    ...
    <footer>
        <p>Bericht automatisch generiert von Piwigo-Export-Skript · JeromeX</p>
    </footer>
</body>
</html>""")
